...
--							Dropdowns							--
------------------------------------------------------------------
-- Remove Set/Clear Focus buttons
do
	local _, class = UnitClass("player")
	for _, menu in pairs(UnitPopupMenus) do
		for i = #menu, 1, -1 do
			local name = menu[i]
			if name == "SET_FOCUS" or name == "CLEAR_FOCUS" or (class == "HUNTER" and name == "PET_DISMISS") then
				tremove(menu, i)
			end
		end
	end
...
function ns.SpawnDropDownMenu(self)
	local unit = self.unit:sub(1, -2)
	if unit == "party" then
		ToggleDropDownMenu(1, "PARTY", _G["PartyMemberFrame" .. self.id .. "DropDown"], "cursor")
	elseif (unit == "boss" ) then -- Need check, probably work
		ToggleDropDownMenu(1, "BOSS", _G["Boss"..self.id.."TargetFrameDropDown"], "cursor")
	else
...
UnitPopupButtons["RESET_FOCUS_FRAME_POSITION"] = { text = RESET_POSITION, dist = 0 };
UnitPopupMenus['MOVE_FOCUS_FRAME'] = { 'UNLOCK_FOCUS_FRAME', 'LOCK_FOCUS_FRAME', 'RESET_FOCUS_FRAME_POSITION' }
...
hooksecurefunc("UnitPopup_OnClick", function(self)
    ...
    if ( button == "RESET_FOCUS_FRAME_POSITION" ) then
        FocusFrame_ResetUserPlacedPosition();
    end
end)
